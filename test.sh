...
if g++ --version > /dev/null; then
    ...
    TestStage "./build/string" "Tests finished successfully!" "Tests failed"
else
...
if clang++ --version > /dev/null; then
    ...
    if clang-tidy --version > /dev/null; then
        TestStage "clang-tidy string.h" "Codestyle check successful!" "First codestyle check failed"
    else
    ...
    #    indent=4
    #    if cat string.h | grep "^  String" > /dev/null; then
    #        indent=2
    #    fi
